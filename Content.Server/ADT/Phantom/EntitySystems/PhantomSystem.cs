...
public sealed partial class PhantomSystem : EntitySystem
{
    ...
    private void OnBreakdown(EntityUid uid, PhantomComponent component, BreakdownActionEvent args)
    {
        ...
        if (IsHolder(target, component.PhantomBreakdownActionEntity, component))
        {
            if (TryComp<StatusEffectsComponent>(target, out var status) && HasComp<BatteryComponent>(target))
            {
                if (!component.BreakdownOn)
                {
        ...
        else
        {
            ...
            if (HasComp<StatusEffectsComponent>(target) && HasComp<PowerCellSlotComponent>(target) || HasComp<BatteryComponent>(target))
            {
                _status.TryAddStatusEffect<KnockedDownComponent>(target, "KnockedDown", time, false);
                _status.TryAddStatusEffect<StunnedComponent>(target, "Stun", time, false);
